...
CLASS zcl_llm_00_file_list_local DEFINITION LOCAL FRIENDS lcl_.
...
CLASS lcl_ IMPLEMENTATION.
  ...
  METHOD setup.
    mo_cut ?= zcl_llm_00_file_list_local=>new_from_folder(
      iv_     = 'C:\TEMP\'
      iv_mask = '*.env'
    ).
  ENDMETHOD.
  ...
  METHOD save.
    ...
    DATA(lo_cut) = zcl_llm_00_file_list_local=>new_from_folder(
      iv_     = 'C:\TEMP\'
      iv_mask = '*.TXT'
    ).
...
*    ).
  ...
  METHOD new_from_folder.
    ...
    lo_ = zcl_llm_00_file_list_local=>new_from_folder(
      iv_     = 'C:\TEMP\'
      iv_mask = '*.env'
    ).
    DATA(lt_all) = lo_->get( ).
