...
CLASS zcl_llm_00_pat_list DEFINITION LOCAL FRIENDS lcl_.
...
.
  PRIVATE SECTION.
    DATA:
      mo_cut TYPE REF TO zcl_llm_00_pat_list.  "class under test
...
CLASS lcl_ IMPLEMENTATION.
  ...
  METHOD setup.
    mo_cut ?= zcl_llm_00_pat_list=>new_from_folder(
      iv_     = 'C:\TEMP\@zllm\'
      iv_mask = '*.md'
    ).
  ENDMETHOD.
...
*    ).
  ...
  METHOD new_from_folder.
    DATA lo_ TYPE REF TO zif_llm_00_pat_list.
    lo_ = zcl_llm_00_pat_list=>new_from_folder(
      iv_     = 'C:\TEMP\@ZLLM\'
      iv_mask = '*.md'
    ).
    DATA(lt_all) = lo_->get( ).
