...
CLASS zcl_llm_00_file_list_smw0 DEFINITION LOCAL FRIENDS lcl_.
...
CLASS lcl_ IMPLEMENTATION.
  ...
  METHOD setup.
    mo_cut ?= zcl_llm_00_file_list_smw0=>new_from_package(
      iv_namespace = 'Z'
      iv_     = '$ZLLM_00'
      iv_mask = '*.TXT'
    ).
  ...
  METHOD save.
    ...
    DATA(lo_cut) = zcl_llm_00_file_list_smw0=>new_from_package(
      iv_namespace = 'Z'
      iv_     = '$ZLLM_00'
      iv_mask = '*.TXT'
    ).
...
*    ).
  ...
  METHOD new_from_package.
    ...
    lo_ = zcl_llm_00_file_list_smw0=>new_from_package(
      iv_     = '$ZLLM_00'
      iv_mask = '*.TXT'
    ).
    DATA(lt_all) = lo_->get( ).
