class ZCL_LOG_ANALYSER definition
  public
  create protected .

public section.

  interfaces ZIF_LOG_ANALYSER .

  aliases GET_WORST_BAPIRET2
    for ZIF_LOG_ANALYSER~GET_WORST_BAPIRET2 .
  aliases GET_WORST_MESSAGE
    for ZIF_LOG_ANALYSER~GET_WORST_MESSAGE .
  aliases GET_WORST_MESSAGE_TYPE
    for ZIF_LOG_ANALYSER~GET_WORST_MESSAGE_TYPE .
  aliases HAS_ERROR
    for ZIF_LOG_ANALYSER~HAS_ERROR .
  aliases HAS_SUCCESS_ONLY
    for ZIF_LOG_ANALYSER~HAS_SUCCESS_ONLY .
  aliases HAS_WARNING_OR_BETTER
    for ZIF_LOG_ANALYSER~HAS_WARNING_OR_BETTER .

  class-methods NEW
    importing
      !IO_LOG type ref to zif_log
    returning
      value(RO_) type ref to ZCL_LOG_ANALYSER .
  PROTECTED SECTION.
...
private section.
  ...
  aliases HAS_ANY
    for ZIF_LOG_ANALYSER~HAS_ANY .
  aliases HAS_WARNING
    for ZIF_LOG_ANALYSER~HAS_WARNING .
ENDCLASS.



CLASS ZCL_LOG_ANALYSER IMPLEMENTATION.
  ...
  METHOD zif_log_analyser~get_worst_bapiret2.
    DATA(ls_) = me->zif_log_analyser~get_worst_message(  ).
    rs_-type    = ls_-msgty.
    rs_-id         = ls_-msgid.
  ...
  METHOD zif_log_analyser~get_worst_message.
  ...
  METHOD zif_log_analyser~get_worst_message_type.
    DATA(ls_msg) = me->zif_log_analyser~get_worst_message(  ).
    rv_ = ls_msg-msgty.
  ENDMETHOD.


  METHOD zif_log_analyser~has_any.
  ...
  METHOD zif_log_analyser~has_error.
    CASE me->zif_log_analyser~get_worst_message_type(  ).
      WHEN 'X' OR 'A' OR 'E'.
        rv_ = abap_true.
      WHEN OTHERS.
        CLEAR rv_.
  ...
  METHOD zif_log_analyser~has_success_only.
    CASE me->zif_log_analyser~get_worst_message_type(  ).
      WHEN 'S'.
        rv_ = abap_true.
      WHEN OTHERS.
  ...
  METHOD zif_log_analyser~has_warning.
    CASE me->zif_log_analyser~get_worst_message_type(  ).
      WHEN 'W'.
        rv_ = abap_true.
      WHEN OTHERS.
  ...
  METHOD zif_log_analyser~has_warning_or_better.
    CASE me->zif_log_analyser~get_worst_message_type(  ).
      WHEN 'W' OR 'I' OR 'S'.
        rv_ = abap_true.
      WHEN OTHERS.
  ...
  METHOD zif_log_analyser~if_any_show_most_severe_as.
  ...
  METHOD ZIF_LOG_ANALYSER~IF_ERROR_SHOW_MOST_SEVERE_AS.
